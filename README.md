...
- `references/` — 决策知识库：区域画像、价格框架、看房清单、工具使用方法论
- 数据由外部 skill（如 ok-core-skill）提供，本 skill 只消费数据、做判断

### 为什么这样设计？

1. **解耦**：决策逻辑和数据获取分离。换数据源时不需要改决策 skill
2. **可扩展**：未来可对接 PropertyGuru scraper、99.co API、Google Maps MCP 等任何数据源
...
| 通勤评估 | 步行距离分级 + 租金影响系数 + Shelter 评估 + 高峰拥挤度 |
| 生活便利 | 超市/餐饮/医疗距离评级 + 区域便利度评分 |
| 看房清单 | 噪音(5类) + 朝向 + 设施 + 费用 + 租约条款 20+ 检查项 |
| 决策工具 | Google Maps(4用途) + 各地房产平台 + 交通App + 政府数据 |
...
| references | 2 个文件 | 5 个文件：数据契约 + 决策维度 + 区域画像 + 看房清单 + 工具方法论 |
