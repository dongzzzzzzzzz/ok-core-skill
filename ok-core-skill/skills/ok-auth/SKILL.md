...
# 邮箱密码登录（全自动）
uv run --project <SKILL_DIR> ok-cli login --email "user@example.com" --password "yourpassword"

# 等待用户手动完成 OAuth 登录（如 Google/Facebook/Apple）
uv run --project <SKILL_DIR> ok-cli wait-login --timeout 120
...
返回值：
```json
{"logged_in": true, "user_name": "用户名"}
```
或
```json
{"logged_in": false, "user_name": null}
```
...
```bash
uv run python scripts/cli.py login --email "user@example.com" --password "Pass1234"
```
